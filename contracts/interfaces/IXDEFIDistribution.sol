...
interface IXDEFIDistribution is IERC721Enumerable {
    ...
    function distributableXDEFI()
        external
        view
        returns (uint256 distributableXDEFI_);

    /// @notice The amount of XDEFI that was deposited by all currently locked positions.
    function totalDepositedXDEFI()
        external
        view
        returns (uint256 totalDepositedXDEFI_);
    ...
    function positionOf(uint256 id_)
        external
        view
        returns (
            uint96 units_,
    ...
    function bonusMultiplierOf(uint256 duration_)
        external
        view
        returns (uint8 bonusMultiplier_);
