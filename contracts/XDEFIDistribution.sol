...
contract XDEFIDistribution is IXDEFIDistribution, ERC721Enumerable {
    // @audit all constant should be ALL_CAPS. MAX_TOTAL_XDEFI_SUPPLY shouldnt be caps if not constant.
    uint88 internal MAX_TOTAL_XDEFI_SUPPLY =
        uint88(240_000_000_000_000_000_000_000_000);
    ...
    constructor(
        ...
        uint256 zeroDurationPointBase_
    ) ERC721("Locked XDEFI", "lXDEFI") {
    ...
    // @audit onlyOwner should rather be inherited from Ownable
    // rather than copy-pasted to leverage future contract updates.
    modifier onlyOwner() {
        require(owner == msg.sender, "NOT_OWNER");
        _;
    }
    // @audit noReenter should be inherited from ReentrancyGuard rather than copy-pasted.
    modifier noReenter() {
    ...
    //@audit no checks for 0address, if owner makes a mistake he might not understand
    // why the ownership cant be transfered. Solution: check for 0address.
    ...
    //@audit check for duration array of size 0 to avoid useless LockPeriods.
    //@audit check for multipliers array of 0 to avoid mistakes and no rewards given out.
    ...
    /* Position Functions */
    /**********************/
    //@audit check for 0address and 0 amount
    // @audit destination must be msg.sender to avoid loss of funds.
    //@audit missing event
    function lock(
        uint256 amount_,
        uint256 duration_,
        address destination_
    ) external noReenter returns (uint256 tokenId_) {
    ...
    /// @audit check destination ofr 0address
    function unlock(uint256 tokenId_, address destination_)
        external
        noReenter
    ...
    // @audit can be made external
    function withdrawableOf(uint256 tokenId_)
        public
        view
        returns (uint256 withdrawableXDEFI_)
    {
        ...
        return
            _withdrawableGiven(
                position.units,
                position.depositedXDEFI,
                position.pointsCorrection
            );
    }
    ...
    function unlockBatch(uint256[] memory tokenIds_, address destination_)
        external
        noReenter
        returns (uint256 amountUnlocked_)
    {
    ...
    function getPoints(uint256 amount_, uint256 duration_)
        external
        view
        ...
        returns (uint256 tokenId_)
    {
        ...
        for (uint256 i; i < count; ++i) {
            ...
            require(
                positionOf[tokenId].expiry == uint32(0),
                "POSITION_NOT_UNLOCKED"
            );
    ...
    function pointsOf(uint256 tokenId_)
        external
        view
        returns (uint256 points_)
    {
    ...
    function tokenURI(uint256 tokenId_)
        public
        view
        override(IXDEFIDistribution, ERC721)
    ...
    function _generateNewTokenId(uint256 points_)
        internal
        view
        returns (uint256 tokenId_)
    {
    ...
    function _getPoints(uint256 amount_, uint256 duration_)
        internal
        view
        returns (uint256 points_)
    {
    ...
    function _getPointsFromTokenId(uint256 tokenId_)
        internal
        pure
        returns (uint256 points_)
    {
    ...
    /// @audit no check for destination 0address, and max duration?
    function _lock(
        uint256 amount_,
        uint256 duration_,
        address destination_
        ...
        require(
            amount_ != uint256(0) && amount_ <= MAX_TOTAL_XDEFI_SUPPLY,
            "INVALID_AMOUNT"
        );
        ...
        _safeMint(
            destination_,
            tokenId_ = _generateNewTokenId(_getPoints(amount_, duration_))
        );
        ...
        // Create Position.
        uint96 units = uint96(
            (amount_ * uint256(bonusMultiplier)) / uint256(100)
        );
        totalUnits += units;
    ...
    /// @audit check ofr 0address in previous functions.
    function _unlock(address account_, uint256 tokenId_)
        internal
        returns (uint256 amountUnlocked_)
    {
        ...
        amountUnlocked_ = _withdrawableGiven(
            units,
            depositedXDEFI,
            position.pointsCorrection
        );
    ...
    function _unlockBatch(address account_, uint256[] memory tokenIds_)
        internal
        returns (uint256 amountUnlocked_)
    {
        uint256 count = tokenIds_.length;
        ...
        returns (int256 newFundsTokenBalance_)
    {
        ...
        return
            _toInt256Safe(currentDistributableXDEFI) -
        ...
        return
            (_toUint256Safe(
                _toInt256Safe(_pointsPerUnit * uint256(units_)) +
                    pointsCorrection_
            ) / _pointsMultiplier) + uint256(depositedXDEFI_);
    }
}
